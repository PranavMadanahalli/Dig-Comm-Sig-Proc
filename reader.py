...
(nchannels, sampwidth, sampformat, framerate) = (1, 2, pyaudio.paFloat32, 44100)
...
def dominatfreq(time, recorder):
    ...
    #print(codecs.getdecoder(tab))
    print(tab.decode('utf-8',errors='ignore'))
    if (len(tab) == 0):
        return -1
    halftab = np.fft.fft(np.array(tab))
...
p = pyaudio.PyAudio()
recorder = p.open(format=pyaudio.paFloat32, channels=1, rate=44100, input=True,frames_per_buffer=CHUNK)
print("* recording")
